...
namespace aspect
{
  namespace BoundaryFluidPressure
  {
    ...
    {
      const std::shared_ptr<const MaterialModel::MeltOutputs<dim>> melt_outputs = material_model_outputs.template get_additional_output_object<MaterialModel::MeltOutputs<dim>>();
      ...
      for (unsigned int q = 0; q < fluid_pressure_gradient_outputs.size(); ++q)
      {
        ...
        case DensityFormulation::average_density:
        {
          ...
          fluid_pressure_gradient_outputs[q] = ((1.0 - phi) * material_model_outputs.densities[q] * gravity + phi * melt_outputs->fluid_densities[q] * gravity) * normal_vectors[q];
          break;
        }

        case DensityFormulation::weighted_density:
        {
          fluid_pressure_gradient_outputs[q] = ((1.0 - fluid_density_weight) * material_model_outputs.densities[q] * gravity + fluid_density_weight * melt_outputs->fluid_densities[q] * gravity) * normal_vectors[q];
          break;
        }
    ...
    Density<dim>::initialize()
    {
      ...
      {
        side_extraction_boundary_id =
            this->get_geometry_model().translate_symbolic_boundary_name_to_id(side_extraction_boundary_name);
      }
    }
    ...
    Density<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Boundary fluid pressure model");
      {
        prm.enter_subsection("Density");
        {
          ...
          {
            prm.declare_entry("Side extraction boundary", "left",
                              Patterns::Anything(),
                              "Symbolic name of the extraction boundary ('left','right','front', or 'back').");
            prm.enter_subsection("Side boundary extraction indicator function");
            {
      ...
      prm.enter_subsection("Boundary fluid pressure model");
      {
        prm.enter_subsection("Density");
        {
          ...
          else if (prm.get("Density formulation") == "side boundary magma extraction")
          {
            ...
            prm.enter_subsection("Side boundary magma extraction");
            {
              side_extraction_boundary_name = prm.get("Side extraction boundary");
              prm.enter_subsection("Side boundary extraction indicator function");
              {
                try
                {
                  side_extraction_indicator_function.parse_parameters(prm);
                }
                catch (...)
                {
                  std::cerr << "ERROR: FunctionParser failed to parse\n"
                            << "\t'Side boundary magma extraction/Extraction zone indicator function'\n";
                  throw;
                }
              }
              prm.leave_subsection();
