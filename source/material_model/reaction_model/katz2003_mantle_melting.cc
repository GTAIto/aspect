    ...
    namespace ReactionModel
    {
                         ...
                         const NonlinearDependence::Dependence dependence) const
      {
        ...
        if (temperature > T_solidus && temperature < T_liquidus && pressure < 1.3e10)
        {
          ...
          double melt_fraction_derivative_temperature = beta * std::pow((temperature - T_solidus) / (T_lherz_liquidus - T_solidus), beta - 1) / (T_lherz_liquidus - T_solidus);

          double melt_fraction_derivative_pressure = beta * std::pow((temperature - T_solidus) / (T_lherz_liquidus - T_solidus), beta - 1) * (dT_solidus_dp * (temperature - T_lherz_liquidus) + dT_lherz_liquidus_dp * (T_solidus - temperature)) / Utilities::fixed_power<2>(T_lherz_liquidus - T_solidus);
          ...
          if (melt_fraction(temperature, pressure) > F_max)
          {
            ...
            const double dT_max_dp = dT_solidus_dp + 1.0 / beta * std::pow(F_max, 1.0 / beta - 1.0) * dF_max_dp * (T_lherz_liquidus - T_solidus) + std::pow(F_max, 1.0 / beta) * (dT_lherz_liquidus_dp - dT_solidus_dp);

            melt_fraction_derivative_temperature = (1.0 - F_max) * beta * std::pow((temperature - T_max) / (T_liquidus - T_max), beta - 1) / (T_liquidus - T_max);

            melt_fraction_derivative_pressure = dF_max_dp - dF_max_dp * std::pow((temperature - T_max) / (T_liquidus - T_max), beta) + (1.0 - F_max) * beta * std::pow((temperature - T_max) / (T_liquidus - T_max), beta - 1) * (dT_max_dp * (T_max - T_liquidus) - (dT_liquidus_dp - dT_max_dp) * (temperature - T_max)) / Utilities::fixed_power<2>(T_liquidus - T_max);
          }
        ...
        for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
        {
          ...
          if (this->include_melt_transport() && in.requests_property(MaterialProperties::reaction_rates))
          {
            ...
            else
            {
              ...
              // For freezing, we have to multiply with the melting time scale to obtain the mass fraction change
              // that happens in the time defined by the melting time scale (as opposed to a rate).
              // This is later divided by the melting time scale to obtain the rate of melting or freezing, 
              // which is used in the operator splitting scheme.

              if (in_magma_extraction_channel && porosity_change < 0)
              {
                porosity_change = 0.0;
                depletion_change = 0.0;
              }
                                  ...
                                  const double reference_T) const
      {
        ...
        if (melt_out != nullptr && in.requests_property(MaterialProperties::additional_outputs))
        {
          ...
          for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
          {
            ...
            if (this->include_adiabatic_heating())
            {
              ...
              temperature_dependence -= (in.temperature[i] - this->get_adiabatic_conditions().temperature(in.position[i])) * out.thermal_expansion_coefficients[i];
            }
            else
        ...
        if (this->include_melt_transport() && in.requests_property(MaterialProperties::viscosity))
        {
          ...
          {
            for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
              if (magma_extraction_channel_indicator_function.value(in.position[i]) > 0.5 && in.temperature[i] <= channel_base_temperature)
      ...
      Katz2003MantleMelting<dim>::declare_parameters(ParameterHandler &prm)
      {
        ...
        prm.enter_subsection("Magma extraction channel indicator function");
        {
          ...
          prm.declare_entry("Channel base temperature", "1.0e6",
                            Patterns::Double(),
                            ...
                            "points at or below are part of the channel. A value exceeding the "
                            "maximum temperature in the model effectively puts the base at the bottom "
                            "boundary specified by the channel function.");
      ...
      Katz2003MantleMelting<dim>::parse_parameters(ParameterHandler &prm)
      {
        ...
        if (this->get_parameters().reaction_solver_type == Parameters<dim>::ReactionSolverType::fixed_step)
        {
          ...
          AssertThrow(freezing_rate * this->get_parameters().reaction_time_step <= 1.0,
                      ExcMessage("The reaction time step " + Utilities::to_string(this->get_parameters().reaction_time_step) + " in the operator splitting scheme is too large to compute freezing rates! "
      ...
      template <int dim>
      void
      Katz2003MantleMelting<dim>::update()
      {
        if (this->convert_output_to_years())
          magma_extraction_channel_indicator_function.set_time(this->get_time() / year_in_seconds);
        else
          magma_extraction_channel_indicator_function.set_time(this->get_time());
      }
